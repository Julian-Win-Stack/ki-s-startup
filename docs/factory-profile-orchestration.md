...
Scope: How `/factory` uses profiles as the operator-facing orchestration layer, how profiles are resolved, how work is dispatched, and how this layer is customized per repo
...
`/factory` is no longer a dashboard-first objective screen.

It is a chat-first orchestration surface where the operator talks to a selected Factory profile. That profile is a repo-customizable package made of:
...
The current `/factory` design is a customizable orchestration shell built around profiles, not a fixed dashboard.
